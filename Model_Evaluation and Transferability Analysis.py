...
doc = Document()
doc.add_heading("Transferability Results – NY Model", level=1)
...
plt.title("Comparison of XGBoost and Random Forest Predictions – New York", fontsize=13)
plt.xlabel("Actual GVW (kN)", fontsize=12)
plt.ylabel("Predicted GVW (kN)", fontsize=12)
plt.legend()
...
plt.text(min_val + 10, max_val - 25,
         f"XGBoost → RMSE: {rmse_xgb:.2f} kN, R²: {r2_xgb:.3f}\n"
         f"Random Forest → RMSE: {rmse_rf:.2f} kN, R²: {r2_rf:.3f}",
...
plt.title("Comparison of XGBoost and Random Forest Predictions – California", fontsize=13)
plt.xlabel("Actual GVW (kN)", fontsize=12)
plt.ylabel("Predicted GVW (kN)", fontsize=12)
plt.legend()
plt.grid(True, linestyle=':', alpha=0.7)
...
plt.title("Transferability – California: XGBoost", fontsize=12)
plt.xlabel("Actual GVW (kN)")
plt.ylabel("Predicted GVW (kN)")
plt.legend()
plt.grid(True, linestyle=':', alpha=0.7)
...
plt.plot([mn,mx],[mn,mx], color='black', linestyle='-', linewidth=2, label="Perfect Fit (y=x)")
plt.title("Transferability – California: Random Forest", fontsize=12)
plt.xlabel("Actual GVW (kN)")
plt.ylabel("Predicted GVW (kN)")
plt.legend()
plt.grid(True, linestyle=':', alpha=0.7)
...
plt.plot([min_val, max_val], [min_val, max_val],
         color='black', linestyle='-', linewidth=2, label='Perfect Fit (y=x)')
plt.title("Linear Regression Model – Texas Dataset", fontsize=13)
plt.xlabel("Actual GVW (kN)", fontsize=12)
plt.ylabel("Predicted GVW (kN)", fontsize=12)
plt.legend()
plt.grid(True, linestyle=':', alpha=0.7)
...
print("\n💾 Results saved as 'LinearRegression_Transferability.csv'")
...
for ax, (df, name) in zip(axes, [(ca, "California"), (tx, "Texas")]):
    y_true_lr, y_pred_lr = pred_and_metrics(lr, df,    f"Linear-Regression → {name}")
    y_true_rf, y_pred_rf = pred_and_metrics(rf, df,    f"Random-Forest → {name}")
    y_true_xgb, y_pred_xgb = pred_and_metrics(xgb, df, f"XGBoost → {name}")
    ...
    ax.set_title(f"Transferability – {name}", fontsize=13)
...
plt.bar(x - bar_width/2, ny_ca, bar_width,
        label="NY → CA", color=color_ny_ca, edgecolor='black')
plt.bar(x + bar_width/2, ny_tx, bar_width,
        label="NY → TX", color=color_ny_tx, edgecolor='black')
