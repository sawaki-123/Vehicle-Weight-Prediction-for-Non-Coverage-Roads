...
plt.title("RMSE per Vehicle Class (Lanes 1–3)")
plt.show()
...
points = [
    ...
    "Ensure consistency across states’ datasets",
]
for point in points:
...
content = slide.placeholders[1].text_frame
steps = [
    "Step 1: Exploratory Data Analysis (EDA – correlation heatmaps, scatter plots)",
...
flow_steps = [
    ...
    ("Data Cleaning (filter classes 4–13, remove GVW=0, align columns)", RGBColor(112, 173, 71)),
...
for point in [
    "GVW ↔ Vehicle Class → Strong positive correlation",
    "GVW ↔ Number of Axles → Strong positive correlation",
    "GVW ↔ Hour → Weak correlation, kept for testing",
]:
    p = content.add_paragraph()
    p.text = "• " + point
...
flow_steps = [
    ("Data Collection (NY, CA, TX)", RGBColor(91, 155, 213)),
    ("Data Cleaning (filter class 4–13, remove GVW=0, align columns)", RGBColor(112, 173, 71)),
    ("Feature Selection (EDA, correlation analysis, expert judgment)", RGBColor(255, 192, 0)),
...
plt.title("Correlation Heatmap — California (Aggregated, 2021)", fontsize=14, pad=12)
plt.tight_layout()
...
plt.title("Correlation Heatmap — New York (Aggregated, 2021)", fontsize=14, pad=12)
plt.xlabel("Variables", fontsize=12)
plt.ylabel("Variables", fontsize=12)
